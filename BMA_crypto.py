...
def encrypt(u, G, N, sigma):
    ...
    gaussian = RealDistribution('gaussian', sigma**2)
...
for sigma in Sigma:
    ...
    for l in range(l_start, l_max+1):
        ...
        for j in range(sim):
            ...
            c = [randint(0,1) for i in xrange(N)]
            # c = [0 for i in xrange(N)]
            r = [w[i]+ 1.0 - 2*c[i] for i in xrange(N)]
            r =  [abs(r[i]) for i in xrange(N)]
            ...
            # for pattern in it.combinations(range(size,k+l), weight1 +1):
            #     log_sum = 0
            #     for itera in range(weight1+1):
            #         log_sum -= llr[pattern[itera]]
            #     sum2_hstern += exp(log_sum)
            ...
            # PA_hstern += P1_hstern*sum1_hstern * P2_hstern*sum2_hstern
            # # The probability of hard stern to find an invertible matrix is closest to 1.

            # I0 = np.argsort([-abs(r[i]) for i in range(N)])
            r = np.sort([-abs(r[i]) for i in range(N)])
            llr = 2*r/sigma**2
            # print I
            # print r[I0[0]], r[I0[1]], r[I0[2]], r[I0[4]]
            I = np.random.permutation(range(k + l)) # Random permutation of the k + l most reliable positions
            # print I_MBA
            # I = [I0[I_MBA[i]] for i in range(k+l)]

            # print [llr[I0[i]] for i in range(100)]

            # Calculate P1 and P2
            P1_MBA = np.prod(1.0/(1.0 + exp(-abs(llr[I[0:floor((k + l)/2)]]))))
            # We need the assumption that the codeword should be uniformly distributed.
            P2_MBA = np.prod(1/(1 + exp(-abs(llr[I[floor((k + l)/2):k + l]]))))
            # print P1_MBA* P2_MBA

            # P_MBA = RR(1.0)
            # for i in range(k+l):
            #     P_MBA /= RR((1.0 + exp(-abs(llr[i]))))
            # print P_MBA
            ...
            for pattern in it.combinations(I[size:k+l], weight2 +1):
                log_sum = 0
                for itera in range(weight1+1):
                    log_sum -= abs(llr[pattern[itera]])
                sum2_MBA += exp(log_sum)
                ...
                if num_curr <= 0:
                    # print num_curr
                    break
            PA_MBA_TEMP = min(1, Ql * P1_MBA*sum1_MBA * P2_MBA*sum2_MBA)
            PA_MBA +=  PA_MBA_TEMP
        ...
        # print(PA_hstern)
        print(PA_MBA)
        print np.log2(PA_MBA)
        log_com_temp = np.log2(T/PA_MBA)
